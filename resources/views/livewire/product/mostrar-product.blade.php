              ...
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                  ...
                  <tbody>
                      @forelse ($products as $product)
                          <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                              ...
                              <td class="flex justify-center px-6 py-4 space-x-4 text-center">
                                <button wire:click="updateproduct({{ $product->id }})" class="flex justify-center gap-2 px-2 text-xs font-bold text-black uppercase bg-white border-black rounded-lg hover:scale-90 ease-in duration-200">
                                ...
                                <button wire:click="confirmDeleteProduct({{ $product->id }})" class="flex justify-center gap-2 px-2 py-2 text-xs font-bold text-white uppercase bg-red-700 rounded-lg hover:scale-90 ease-in duration-200">
                      ...
                      <x-confirmation-modal wire:model="confirmingDeletion">
                        ...
                        <x-slot name="content">
                            ¿Está seguro de que desea eliminar este registro? Una vez que se elimine, todos los datos de este registro se eliminarán de forma permanente.
                        </x-slot>
                    ...
                    {{-- Modal para editar --}}
                    <x-dialog-modal wire:model="editProduct">
                        <x-slot name="title">
                            Editar Producto
                        </x-slot>
                        ...
                        <x-slot name="content">
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6">
                                    ...
                                    <x-input id="name" type="text" class="block w-full mt-1" wire:model="editProductData.name" />
                                </div>
                    
                                <div class="col-span-6">
                                    <x-label for="description" value="Descripción" />
                                    <x-input id="description" type="text" class="block w-full mt-1" wire:model="editProductData.description" />
                                </div>
                    
                                <div class="col-span-6">
                                    <x-label for="price" value="Precio" />
                                    <x-input id="price" type="number" class="block w-full mt-1" wire:model="editProductData.price" />
                                </div>
                    
                                <div class="col-span-6">
                                    <x-label for="stock" value="Stock" />
                                    <x-input id="stock" type="number" class="block w-full mt-1" wire:model="editProductData.stock" />
                                </div>
                            </div>
                        </x-slot>
                    
                        <x-slot name="footer">
                            <x-secondary-button wire:click="$toggle('editProduct')" wire:loading.attr="disabled">
                                Cancelar
                            </x-secondary-button>
